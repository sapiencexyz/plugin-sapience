...
export const submitTransactionAction: Action = {
  ...
  description:
    "Submits a transaction to the blockchain using the provided calldata and contract address. Supports JSON transaction objects with 'to' and 'data' fields, or raw call data with a target address. Uses fuzzy search to intelligently identify call data and target addresses from natural language messages.",
  ...
  handler: async (
    ...
    try {
      ...
      if (!privateKey) {
        throw new Error(
          'No private key found in environment variables or settings'
        );
      }
      ...
      if (jsonMatch) {
        try {
          ...
          throw new Error(
            "Found transaction-like data but couldn't parse as JSON"
          );
        }
      } else {
        ...
        try {
          ...
          if (hexMatches.length > 0) {
            ...
            if (callData && targetAddress) {
              ...
              const addressHints = ['to ', 'address ', 'contract ', 'target '];
              ...
              if (targetAddress) {
                ...
                throw new Error(
                  `Found call data (${callData.substring(
                    0,
                    10
                  )}...) but no target address. Please include the target address in your message.`
                );
              }
            } else {
              throw new Error(
                'Could not identify valid call data in the message'
              );
            }
          } else {
      ...
      if (!transactionData.to || !transactionData.data) {
        throw new Error(
          "Transaction data must include 'to' address and 'data' field"
        );
      }
      ...
      if (formattedPrivateKey.length !== 66) {
        throw new Error(
          `Invalid private key format. Expected 66 characters (0x + 64 hex), got ${
            formattedPrivateKey.length
          } characters. Current value appears to be: ${
            formattedPrivateKey.length === 42 ? 'an address' : 'invalid format'
          }`
        );
      }
      ...
      if (!/^0x[a-fA-F0-9]{64}$/.test(formattedPrivateKey)) {
        throw new Error(
          'Invalid private key format. Must be a valid hex string with 64 characters after 0x prefix'
        );
      }
      ...
      const chainName =
        runtime.getSetting('CHAIN') || process.env.CHAIN || 'arbitrum';
      ...
      logger.info(
        `Submitting transaction on ${chain.name} to ${transactionData.to} from ${account.address}`
      );
